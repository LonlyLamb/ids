...
INIT_DISTANCE = 300  #初期（侵入者がいない時）の距離cm
DIST_RATIO = 0.9 #侵入者と判断する距離の割合
...
def my_callback_detect(SENSOR_PIN):
    ...
    if time.time() - last_time >= INTERVAL:
        ...
        """
        sv = calc_sonic_velocity(TEMPERATURE)
        if calc_distance(TRIG_PIN, ECHO_PIN, 5, sv) < INIT_DISTANCE * DIST_RATIO:
